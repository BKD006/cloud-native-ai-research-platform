...
from src.workflow.interview_workflow import InterviewGraphBuilder
from src.prompt_library.prompt_lib import (
...
class AutonomousReportGenerator:
    ...
    def finalize_report(self, state: ResearchGraphState):
        """
        Assemble the introduction, content and conclusion in the final report.
        """
        try:
    ...
    def _save_as_docx(self, text: str, file_path: str):
        ...
        try:
            ...
            self.logger.error("Docx save filed", path=file_path, error=str(e))
            raise ResearchAnalystException("Error saving Docx report", e)

    def _save_as_pdf(self, text: str, file_path: str):
        """
        Save as pdf with centered block, wrapping, and clean layout.
        """
        try:
    ...
    def save_report(self, final_report: str, topic:str, format: str="docx"):
        """
        Save the report in docx and pdf format in its own folder.
        """
        try:
